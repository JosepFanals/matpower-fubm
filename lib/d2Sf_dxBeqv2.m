...
if vcart
    ...
    for k=1:nBeqv 
        ...
        d2Sf_dBeqvVa(:, k) = ((diag(Cf*V)*conj(dYf_dBeq*jdiagV   )   +   Cf.*(Cf*1j*V     ).*conj(dYf_dBeq*V)).')*mu; %AAB- Final dSf_dVaBeq has a size of [nl, nBeqx] 
        d2Sf_dBeqvVm(:, k) = ((diag(Cf*V)*conj(dYf_dBeq*diagVnorm)   +   Cf.*(Cf*diagVnorm).*conj(dYf_dBeq*V)).')*mu; %AAB- Final dSf_dVmBeq has a size of [nl, nBeqx] 
