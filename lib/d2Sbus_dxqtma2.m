...
if vcart
    ...
    for k=1:nQtma 
        ...
        for kk=1:nBeqz
            ...
            d2Yff_dmaBeqz = (-2j*qtmaBeqzSel2)./( (k2.^2).*((abs(tap)).^3) );    %AAB-%Original: d2Yff_dmaBeqz = zeros(nl,1);
            ...
            d2Ybus_dmaBeqz = Cf'*(d2Yf_dmaBeqz) + Ct'*(d2Yt_dmaBeqz);      %AAB- must be zero
            d2Sbus_dqtmaBeqz(kk,k) = (V.*conj(d2Ybus_dmaBeqz*V)).'*lam;      %AAB- must be zero           
        end
        for kk=1:nBeqv
            qtmaBeqvSel=diagqtmaBeqvSel(:,iBeqv(kk)); %AAB- Selects the column of diagqtmaBeqvSel representing only the active element controlling Vf and Qt with Beq and ma
            ...
            d2Sbus_dqtmaBeqv(kk,k) = (V.*conj(d2Ybus_dmaBeqv*V)).'*lam;      %AAB- must be zero           
        end
        for kk=1:nPfsh
            YsmashSel2=diagYsmashSel(:,iPfsh(kk)); %AAB- Selects the column of diagmashSel representing only the active element controlling Pf and Qt with sh and ma
