function [V, branch, converged, i] = newtonpf_aab(branch, Sbus, getYbus, V0, ref, pv, pq, baseMVA, mpopt)
%NEWTONPF_AAB  Solves the AC/DC power flow using a full Newton's method.
%   [V, BRANCH, CONVERGED, I] = NEWTONPF_AAB(BRANCH, SBUS, GETYBUS, V0, REF, PV, PQ, MPOPT)
%
...
%% options
tol        = mpopt.pf.tol;
max_it     = mpopt.pf.nr.max_it; %AAB- Maximum number of iterations
if max_it < 25
    max_it = 25; %NR with FUBM controls or AC/DC grids sometimes takes more than 4 iterations. 
end
lin_solver  = mpopt.pf.nr.lin_solver;
%AAB-----------------------------------------------------------------------
mpopt.pf.nr.conv_graph = 1;           %AAB- 1 if plot convergence graphs 0 if not
conv_graph  = mpopt.pf.nr.conv_graph; %AAB- Plot convergence graph option
%--------------------------------------------------------------------------
%% Identify elements
%%AAB----------------------------------------------------------------------
%%Identify power control elements (Converters and transformers included)
iBeqz = find( (branch(:,CONV)==1) & (branch(:,BR_STATUS)~=0) ); %AAB- Location of the branch elements with Qf control by Beq to meet the zero constraint. (Converters type I for zero constraint control) Qf = 0
iPfsh = find( (branch(:,PF  )~=0) & (branch(:,BR_STATUS)~=0) & (branch(:, SH_MIN )~=-360 | branch(:, SH_MAX )~=360)); %AAB- Location of the branch elements with Pf control by theta_shift to meet the setting. (Converters and Phase Shifter Transformers)
iPtsh = find( (branch(:,PT  )~=0) & (branch(:,BR_STATUS)~=0) & (branch(:, SH_MIN )~=-360 | branch(:, SH_MAX )~=360)); %AAB- Location of the branch elements with Pt control by theta_shift to meet the setting. (Converters and Phase Shifter Transformers)
iQfma = find( (branch(:,QF  )~=0) & (branch(:,BR_STATUS)~=0) & (branch(:, TAP_MIN)~= branch(:, TAP_MAX)) & branch(:,VF_SET)==0 );           %AAB- Location of the branch elements with Qf control by ma to meet the setting. (Transformers)
iQtma = find( (branch(:,QT  )~=0) & (branch(:,BR_STATUS)~=0) & (branch(:, TAP_MIN)~= branch(:, TAP_MAX)) & branch(:,VT_SET)==0 );           %AAB- Location of the branch elements with Qt control by ma to meet the setting. (Converters and Transformers)
...
iBeqv = find( (branch(:,CONV)==2) & (branch(:,BR_STATUS)~=0) & (branch(:,VF_SET)~=0) ); %AAB- Location of the branch elements with Vf control by Beq to meet the setting. (Converters type II for Vdc control) Vf = Vfset
iVtma = find((branch(:,BR_STATUS)~=0) & (branch(:,VT_SET)~=0) & (branch(:, TAP_MIN)~= branch(:, TAP_MAX)) ); %AAB- Location of the branch elements with Vt control by ma  to meet the setting. (Converters and Transformers)
VfBeqbus = branch(iBeqv, F_BUS); %AAB- Saves the "from" bus identifier for Vf controlled by Beq (Converters type II for Vdc control)
Vtmabus  = branch(iVtma , T_BUS); %AAB- Saves the "to"   bus identifier for Vt controlled by ma  (Converters and Transformers)
...
%% initialize V
converged = 0; %AAB- Convergence flag
i = 0;         %AAB- iterations
V = V0;        %AAB- V  initial condition
Va = angle(V); %AAB- Va initial condition
Vm = abs(V);   %AAB- Vm initial condition
...
%% variable dimensions
%%AAB-----------------------------------------------------------------------
nPfsh = length(iPfsh); %AAB- number of Pf controlled elements by theta_shift
nQfma = length(iQfma); %AAB- number of Qf controlled elements by ma
nQtma = length(iQtma); %AAB- number of Qt controlled elements by ma
nVtma = length(iVtma); %AAB- number of Vt controlled elements by ma
nBeqz = length(iBeqz); %AAB- number of Qf controlled elements by Beq
nBeqv = length(iBeqv); %AAB- number of Vf controlled elements by Beq
nVscL = length(iVscL); %AAB- Number of VSC with active PWM Losses Calculation  

nVfBeqbus = length(VfBeqbus); %AAB- number of buses for Vf controlled by Beq 
nVtmabus  = length(Vtmabus ); %AAB- number of buses for Vt controlled by ma  
%--------------------------------------------------------------------------
...
%%AAB----------------------------------------------------------------------
[Ybus, Yf, Yt] = getYbus(branch); %AAB- Obtains Ybus, Yf, Yt from branch
%%-------------------------------------------------------------------------
%% compute branch power flows
%br = find(branch(:, BR_STATUS));  %%AAB- in-service branches
brf=branch(:, F_BUS);              %%AAB- from bus index of all the branches, brf=branch(br, F_BUS); %For in-service branches 
brt=branch(:, T_BUS);              %%AAB- to   bus index of all the branches, brt=branch(br, T_BUS); %For in-service branches
If= Yf(:, :) * V;                  %%AAB- complex current injected at "from" bus, Yf(br, :) * V; For in-service branches 
It= Yt(:, :) * V;                  %%AAB- complex current injected at "to"   bus, Yt(br, :) * V; For in-service branches 
Sf = V(brf) .* conj(If);           %%AAB- complex power injected at "from" bus
St = V(brt) .* conj(It);           %%AAB- complex power injected at "to"   bus
...
if nVscL
    ...
    branch(iVscL,GSW) = PLoss_IEC./(abs(V(brf(iVscL))).^2);    %%AAB- VSC Gsw
end
%% evaluate F(x0)
%%AAB----------------------------------------------------------------------
mis = V .* conj(Ybus * V) - Sbus(Vm);     %AAB- F1(x0) & F2(x0) Power balance mismatch
misPfsh = real(Sf(iPfsh)) - Pfset(iPfsh); %AAB- F3(x0) Pf control mismatch  
misQfma = imag(Sf(iQfma)) - Qfset(iQfma); %AAB- F4(x0) Qf control mismatch 
misBeqz = imag(Sf(iBeqz)) - 0;            %AAB- F5(x0) Qf control mismatch 
misBeqv = imag(mis(VfBeqbus));            %AAB- F6(x0) Vf control mismatch 
misVtma = imag(mis(Vtmabus ));            %AAB- F7(x0) Vt control mismatch 
misQtma = imag(St(iQtma)) - Qtset(iQtma); %AAB- F8(x0) Qt control mismatch 
%%-------------------------------------------------------------------------

%% Create F vector
%%AAB----------------------------------------------------------------------
F = [   real(mis([pv; pq])); %AAB- F1(x0) Power balance mismatch - Va
        imag(mis(pq));       %AAB- F2(x0) Power balance mismatch - Vm
        misPfsh;             %AAB- F3(x0) Pf control    mismatch - Theta_shift
        misQfma;             %AAB- F4(x0) Qf control    mismatch - ma
        misBeqz;             %AAB- F5(x0) Qf control    mismatch - Beq
        misBeqv;             %AAB- F6(x0) Vf control    mismatch - Beq
        misVtma;             %AAB- F7(x0) Vt control    mismatch - ma
        misQtma;             %AAB- F8(x0) Qt control    mismatch - ma
        ];
%%-------------------------------------------------------------------------    

%% check tolerance
normF = norm(F, inf); %AAB- Calculates the mismatch error of the vector F, norm = max( abs(F) ) 
if mpopt.verbose > 1
...
while (~converged && i < max_it) %Repeat until convergence or the maximum iterations limit is reached
    ...
    if conv_graph
        ...
        fmis(i,:)=F';                      %AAB- Saves iterative values (mismatch)
        if nQtma 
            fQtma (i,:)=branch(iQtma,TAP); %AAB- Saves iterative values (tap or VSC modulation amplitude)
        end
        if nVtma 
            fVtma (i,:)=branch(iVtma,TAP); %AAB- Saves iterative values (tap or VSC modulation amplitude)
        end
    end
    %%---------------------------------------------------------------------
    %% evaluate Jacobian
   ...
   %dx = -( J \ F ); %Matpower 6
    ...
    V = Vm .* exp(1j * Va); %%AAB- Reconstruct V
    ...
    if nQfma 
        ...
        for k=1:nQfma
            if  branch(iQfma(k),TAP) > branch(iQfma(k),TAP_MAX)
                branch(iQfma(k),TAP) = branch(iQfma(k),TAP_MAX); %AAB- Fix maximum tap
            elseif branch(iQfma(k),TAP) < branch(iQfma(k),TAP_MIN)
                branch(iQfma(k),TAP) = branch(iQfma(k),TAP_MIN); %AAB- Fix minimum tap
            end
        end
    end
    %%Beqz
    ...
    if nVtma 
        ...
        for k=1:nVtma
            if  branch(iVtma(k),TAP) > branch(iVtma(k),TAP_MAX)
                branch(iVtma(k),TAP) = branch(iVtma(k),TAP_MAX); %AAB- Fix maximum tap
            elseif branch(iVtma(k),TAP) < branch(iVtma(k),TAP_MIN)
                branch(iVtma(k),TAP) = branch(iVtma(k),TAP_MIN); %AAB- Fix minimum tap
            end
        end
    end
    %%Qtma (tap)
    ...
    %%AAB------------------------------------------------------------------
    [Ybus, Yf, Yt] = getYbus(branch); %AAB- Obtains Ybus, Yf, Yt
    %%---------------------------------------------------------------------
%% compute branch power flows
%br = find(branch(:, BR_STATUS));  %%AAB- in-service branches
brf=branch(:, F_BUS);              %%AAB- from bus index of all the branches, brf=branch(br, F_BUS); %For in-service branches 
...
if conv_graph
    ...
    fmis(i+1,:)=F';                      %AAB- Saves iterative values (mismatch)
    if nQtma 
        fQtma (i+1,:)=branch(iQtma,TAP); %AAB- Saves iterative values (tap or VSC modulation amplitude)
    end
    if nVtma 
        fVtma (i+1,:)=branch(iVtma,TAP); %AAB- Saves iterative values (tap or VSC modulation amplitude)
    end
    %Ploting
    Graphsfmis(i, fmis);
    if iQtma
