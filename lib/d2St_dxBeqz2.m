...
if vcart
    ...
    for k=1:nBeqz 
        ...
        d2St_dBeqzVa(:, k) = ((diag(Ct*V)*conj(dYt_dBeq*jdiagV   )   +   Ct.*(Ct*1j*V     ).*conj(dYt_dBeq*V)).')*mu; %AAB- Final dSf_dBeqVa has a size of [nl, nBeqx] 
        d2St_dBeqzVm(:, k) = ((diag(Ct*V)*conj(dYt_dBeq*diagVnorm)   +   Ct.*(Ct*diagVnorm).*conj(dYt_dBeq*V)).')*mu; %AAB- Final dSf_dBeqVm has a size of [nl, nBeqx] 
