...
if vcart
    ...
    diagYttma= sparse( diag(maAux.*Ytt) ); %AAB- ma selector multilied by the series addmitance Ytt, size [nl,nl]
    ...
    for k=1:nXxma
        ...
        Yttma=diagYttma(:,iXxma(k)); %AAB- Selects the column of diagmaAux representing only the active ma for the specified control
        
        %Partials of Ytt, Yff, Yft and Ytf w.r.t. ma
        dYtt_dma(:, k) = sparse( zeros(nl,1) );
        dYff_dma(:, k) = sparse( -2*Yttma./( (k2.^2).*((abs(tap)).^3) ) );
