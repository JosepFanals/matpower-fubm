...
if vcart
    ...
    for k=1:nVtma
        ...
        d2Sf_dQtmaPfsh(:, k) = (dSf_dQtma_PertVtma - dSf_dQtma).' * lam / pert;  %QtmaVtma from, size of [nQtma , nVtma] 
        d2St_dQtmaPfsh(:, k) = (dSt_dQtma_PertVtma - dSt_dQtma).' * lam / pert;  %QtmaVtma  to , size of [nQtma , nVtma] 
    end
    %Vtma2 num_G77
    for k=1:nVtma
        PertSel=diagVtmaSel(:,iVtma(k)); %AAB- Selects the column of diagVtmaSel representing only the active Vtma
