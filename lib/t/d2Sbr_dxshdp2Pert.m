...
if vcart
    ...
    for k=1:nPfdp
        ...
        d2Sf_dQtmaPfsh(:, k) = (dSf_dQtma_PertPfdp - dSf_dQtma).' * lam / pert;  %QtmaPfdp from, size of [nQtma , nPfdp] 
        d2St_dQtmaPfsh(:, k) = (dSt_dQtma_PertPfdp - dSt_dQtma).' * lam / pert;  %QtmaPfdp  to , size of [nQtma , nPfdp] 
    end
    %VtmaPfdp num_G78
    for k=1:nPfdp
        PertSel=diagShdpSel(:,iPfdp(k)); %AAB- Selects the column of diagShdpsel representing only the active Shdp
        ...
        d2Sf_dVtmaPfsh(:, k) = (dSf_dVtma_PertPfdp - dSf_dVtma).' * lam / pert;  %VtmaPfdp from, size of [nVtma , nPfdp] 
        d2St_dVtmaPfsh(:, k) = (dSt_dVtma_PertPfdp - dSt_dVtma).' * lam / pert;  %VtmaPfdp  to , size of [nVtma , nPfdp] 
    end
    %Pfdp2 num_G88
    for k=1:nPfdp
        PertSel=diagShdpSel(:,iPfdp(k)); %AAB- Selects the column of diagShdpsel representing only the active Shdp
